...
function parseCliArguments(argv) {
  if (argv.length === 2) {
    ...
    if (firstOperation === "squareRoot") {
      return {
        operation: first,
        left: parseNumber(second, "value"),
      };
    }

    if (secondOperation === "squareRoot") {
      return {
        operation: second,
        left: parseNumber(first, "value"),
      };
    }

    throw new Error("Expected two arguments only for square root.");
  }

  if (argv.length !== 3) {
    throw new Error("Expected exactly three arguments for binary operations.");
  }
...
function getUsageText() {
  return [
    ...
    "  node src/calculator.js <value> <operation>",
    "",
    "Examples:",
